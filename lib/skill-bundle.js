...
const IDE_DEFINITIONS = {
  antigravity: {
    ...
    detectProjectPaths: ['.agent', 'agent'],
  ...
  vscode: {
    ...
    detectProjectPaths: ['.github/skills', '.vscode'],
  ...
  copilot: {
    ...
    projectRoot: '.github',
    globalRoot: path.join(os.homedir(), '.copilot'),
    detectProjectPaths: ['.github/skills', '.github/copilot-instructions.md'],
    detectGlobalPaths: ['.copilot/skills'],
    targets: [{ relativeDir: 'skills', format: 'skill' }],
  },
  generic: {
...
function detectIDE(baseDir) {
  ...
  for (const candidate of candidates) {
    ...
    for (const detectPath of definition.detectProjectPaths) {
      if (pathExists(baseDir, detectPath)) {
        return candidate;
      }
    }
  }
