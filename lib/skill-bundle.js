...
const IDE_DEFINITIONS = {
  ...
  windsurf: {
    ...
    globalRoot: path.join(os.homedir(), '.codeium', 'windsurf'),
    detectProjectPaths: ['.windsurf', '.windsurf/skills'],
    detectGlobalPaths: ['.codeium/windsurf/skills'],
    targets: [{ relativeDir: 'skills', format: 'skill' }],
    compatibilityProjectRoots: ['.agents'],
  },
  codex: {
    displayName: 'Codex',
    projectRoot: '.agents',
...
function renderSkillFile(skill, format) {
  ...
  if (format === 'workflow') {
    ...
    lines.push('---', '', `Canonical source: .kiro/skills/${skill.slug}/SKILL.md`, '', skill.body, '');
    return lines.join('\n');
  }
...
function findIDEContext(startDir) {
  const candidates = ['antigravity', 'kiro', 'cursor', 'claude-code', 'windsurf', 'codex', 'copilot', 'vscode'];
